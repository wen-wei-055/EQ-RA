...
def calculate_warning_times(config, model_list, data, event_metadata, batch_size, district_PGA_event_idx, sampling_rate=100,
    ...
    for i, _ in tqdm(enumerate(event_metadata.iterrows()), total=len(event_metadata)): 
        ...
        if args.experiment_retrieve_event:
            print('experiment_retrieve_event is ON')
        ...
        generator = util_origin.PreloadedEventGenerator(data=data_tmp, 
                                                 ...
                                                 district_PGA_event_idx=district_PGA_event_idx,
                                                 **generator_params)
        ...
        predictions, survival_list, choose_event_list = model_list.predict_generator(cutout_generator, workers=0, use_multiprocessing=False)  
...
if __name__ == '__main__':
    ...
    if args.val:
        if args.experiment_retrieve_event:
            output_dir = os.path.join(args.experiment_path, f'Experiment_MAE_event_evaluation{suffix}', 'val')
        else:
            output_dir = os.path.join(args.experiment_path, f'evaluation{suffix}', 'val')
        data_path = training_params['total_data_path']
        district_PGA_event_idx = json.load(open(training_params['val_district_PGA_event_idx'],'r'))
        test_set = False
    else:
        if args.experiment_retrieve_event:
            output_dir = os.path.join(args.experiment_path, f'Experiment_MAE_event_evaluation{suffix}', 'test')
        else:
            output_dir = os.path.join(args.experiment_path, f'evaluation{suffix}', 'test')
        data_path = training_params['total_data_path']
        district_PGA_event_idx = json.load(open(training_params['test_district_PGA_event_idx'],'r'))
        test_set = True

    if args.experiment_retrieve_event:
        tmp_file_path = os.path.join(args.experiment_path, f'Experiment_MAE_event_evaluation{suffix}')
    else:
    ...
    if args.head_times:
        ...
        warning_time_information, total_choose_event_list = calculate_warning_times(config, model_list, data, event_metadata,
                                                           times=times_pga,
                                                           alpha=alpha,
