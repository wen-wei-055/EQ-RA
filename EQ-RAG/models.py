...
class TotalEmbedding(nn.Module):
    def __init__(self, input_shape, left_station, right_station,
        ...
        self.LocEmbedding = nn.Embedding(3, embedding_dim=500)  
        nn.init.zeros_(self.LocEmbedding.weight.data)
        
        w = json.load(open('dataset_configs/st_loc_type.json'))
        w = torch.Tensor(list(w.values())).to(self.device).long()
        self.w = w.repeat(2)
...
class EnsembleEvaluateModel:
    ...
    def predict_generator(self, generator, **kwargs):
        ...
        for i, model in enumerate(self.models): 
            ...
            with torch.no_grad():
                for x, y in dataloader: 
                    inputs_waveforms, inputs_coords, survival_list = \
                    ...
                    outputs = model(inputs_waveforms, inputs_coords, survival_list, None) 
                    outputs =  torch.squeeze(outputs[:,:249], 0)
        ...
        return self.merge_preds(preds),  survival_list.cpu().numpy(), choose_event_list 
    ...
    def prediction_generator_partial_station(self, time, event_metadata,
                                             val_bool, generator_params, data,
                                             stations_table, max_stations,
                                             sampling_rate, stations_channel_boolean,
                                             stations_channel_class, latlon_IDtable,
                                             district_PGA_event_idx, topK_nearest_file,
                                             **kwargs):        
        ...
        for i, _ in tqdm(enumerate(event_metadata.iterrows()), total=len(event_metadata)): 
            ...
            generator = util_origin.PreloadedEventGenerator(data=data_tmp, 
                                                    retrieve_event=None,
                                                    experiment_retrieve=None,
                                                    topK_nearest_file_path=topK_nearest_file,
                                                    validation_set=val_bool,
                                                    data_total=data,
                                                    event_id=i,
            ...
            for i, model in enumerate(self.models):
                if i > 0:
                    raise ValueError('self.models number > 1')
                dataloader = DataLoader(generator, batch_size=None, collate_fn=my_collate,num_workers=0)
                output_list = []
                output_label_list = []
                model.eval()
                with torch.no_grad():
                    for x, y in dataloader: 
                        ...
                        outputs = outputs[self.filter_borehole]
                        targets_pga = targets_pga[self.filter_borehole]
                        ...
                        outputs_select[:,:,0] = False
                        outputs_select[:,:,2] = False
